...
import school.objects.MiraReward;
...
public class Ghost extends GameObject{
	...
	public Ghost(float x, float y, Handler handler, Game game, ID id, boolean active) {
		super(x, y, id);
	...
	public void render(Graphics g) {
		if(texTimer >= 60){
			g.drawImage(tex.enemy[13], (int)x, (int)y, 200, 200, null);
	...
	public void die(){
		if(health <= 0){
			this.active = false;
			handler.addObject(new MiraReward(x,y,ID.artifact,handler));
			for(int i = 0; i < handler.object.size(); i++){
				GameObject tempObject = handler.object.get(i);
