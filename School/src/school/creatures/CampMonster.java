...
import school.objects.MattReward;

public class CampMonster extends GameObject{
	...
	public CampMonster(float x, float y, Handler handler, Game game, ID id) {
		super(x, y, id);
	...
	public void render(Graphics g) {
		if(texTimer >= 60){
			g.drawImage(tex.enemy[0], (int)x, (int)y, 32, 32, null);
		}else if(texTimer < 60 && texTimer >= 30){
			g.drawImage(tex.enemy[1], (int)x, (int)y, 32, 32, null);
		}else if(texTimer < 30){
			g.drawImage(tex.enemy[2], (int)x, (int)y, 32, 32, null);
		}
	...
	public void die(){
		if(health <= 0){
			...
			if(handler.enemyCount == 0){
				handler.addObject(new MattReward(x, y, ID.artifact, handler));
				for(int ii = 0; ii < QuestList.Quests.size(); ii++){
					
					if((int)QuestList.Quests.elementAt(ii) == 111){
						QuestList.Quests.remove(ii);
						QuestList.Quests.push(112);
