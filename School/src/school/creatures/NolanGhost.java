...
import school.objects.NathanielReward;

public class NolanGhost extends GameObject{
	...
	public NolanGhost(float x, float y, Handler handler, Game game, ID id, boolean active) {
		super(x, y, id);
	...
	public void render(Graphics g) {
		if(texTimer >= 60){
			g.drawImage(tex.enemy[8], (int)x, (int)y, 200, 200, null);
	...
	public void die(){
		if(health <= 0){
			this.active = false;
			handler.addObject(new NathanielReward(x, y, ID.artifact, handler));

			for(int ii = 0; ii < QuestList.Quests.size(); ii++){
				
				if((int)QuestList.Quests.elementAt(ii) == 123){
