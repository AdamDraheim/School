...
public class GreysonReward extends GameObject{
	...
	Handler handler;
	public GreysonReward(float x, float y, ID id, Handler handler) {
		super(x, y, id);
	...
	public void render(Graphics g) {
		g.drawImage(tex.artifact[6], (int)x,  (int)y, 32, 32, null);
	}
		...
		for(int i = 0; i < handler.object.size(); i++){
			...
			if(tempObject.getId() == ID.player){
				if(tempObject.getBounds().intersects(this.getBounds())){
					Artifacts.Greyson = true;
					for(int ii = 0; ii < QuestList.Quests.size(); ii++){
						
						if((int)QuestList.Quests.elementAt(ii) == 82){
