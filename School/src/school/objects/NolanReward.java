...
public class NolanReward extends GameObject{
	...
	Handler handler;
	public NolanReward(float x, float y, ID id, Handler handler) {
		super(x, y, id);
		...
		for(int i = 0; i < handler.object.size(); i++){
			...
			if(tempObject.getId() == ID.player){
				if(tempObject.getBounds().intersects(this.getBounds())){
					Artifacts.Nolan = true;
					for(int ii = 0; ii < QuestList.Quests.size(); ii++){
						
						if((int)QuestList.Quests.elementAt(ii) == 140){
