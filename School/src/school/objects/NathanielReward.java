...
public class NathanielReward extends GameObject{
	...
	Handler handler;
	public NathanielReward(float x, float y, ID id, Handler handler) {
		super(x, y, id);
	...
	public void render(Graphics g) {
		g.drawImage(tex.artifact[10], (int)x,  (int)y, 32, 32, null);
	}
		...
		for(int i = 0; i < handler.object.size(); i++){
			...
			if(tempObject.getId() == ID.player){
				if(tempObject.getBounds().intersects(this.getBounds())){
					Artifacts.Nathaniel = true;
					for(int ii = 0; ii < QuestList.Quests.size(); ii++){
						
						if((int)QuestList.Quests.elementAt(ii) == 123 || (int)QuestList.Quests.elementAt(ii) == 122){
