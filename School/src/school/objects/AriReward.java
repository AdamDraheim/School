...
public class AriReward extends GameObject{
	...
	Handler handler;
	public AriReward(float x, float y, ID id, Handler handler) {
		super(x, y, id);
	...
	public void render(Graphics g) {
		g.drawImage(tex.artifact[0], (int)x,  (int)y, 32, 32, null);
	}
			...
			if(tempObject.getId() == ID.player){
				if(tempObject.getBounds().intersects(this.getBounds())){
					...
					for(int ii = 0; ii < QuestList.Quests.size(); ii++){
						...
						if((int)QuestList.Quests.elementAt(ii) == 22){
